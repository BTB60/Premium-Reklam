...
export default function OrdersTable() {
  ...
  const updateOrderStatus = async (orderId: number, status: string) => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://premium-reklam-backend.onrender.com"}/api/orders`, {
        method: "PATCH",
        headers: {
  ...
  const deleteOrder = async (orderId: number) => {
    ...
    try {
      const token = JSON.parse(localStorage.getItem("decor_current_user") || "{}")?.token;
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://premium-reklam-backend.onrender.com"}/api/orders?id=${orderId}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`
        }
